  ...
  //  private static final InterfaceC9254c LOGGER = C9255d.m255i("SakeClientImpl");
   ...
   // private final List<Isakeclient.InterfaceC7174a> listeners = new CopyOnWriteArrayList();
    ...
    public SakeClient(byte[] bArr) {
        if (!initClient(bArr)) {
            throw new SakeClientException("Failed to initialize the SAKE client, key database is missing or corrupt.");
        } else {
    ...
    private static SakeClientStatus adaptHandshakeStatus(SakeHandshakeStatus sakeStatus) {
        return sakeStatus == SakeHandshakeStatus.E_SAKE_HANDSHAKE_SUCCESSFUL ? SakeClientStatus.AUTHORIZED : sakeStatus == SakeHandshakeStatus.E_SAKE_HANDSHAKE_IN_PROGRESS ? SakeClientStatus.IN_PROGRESS : sakeStatus == SakeHandshakeStatus.E_SAKE_HANDSHAKE_FAILED ? SakeClientStatus.FAILED : SakeClientStatus.UNAUTHORIZED;
    }
  ...
  //  @Override // p123e.p416g.p471f.p472a.p473d.Sake.Isakeclient
    ...
    public byte[] encrypt(byte[] bArr) {
        ...
        if (z) {
            ...
            z = SakeJNIWrapper.isSecureForSending(this.nativeSakeClient, nativeSakeUserMessage, nativeSakeSecureMessage);
        ...
        if (z) {
            return SakeJNIWrapper.cdataUint(nativeSakeSecureMessage.getBytes(), (int) nativeSakeSecureMessage.getSecureMessageByteCount());
        }
        return null;
    }
  ...
  //  @Override // p123e.p416g.p471f.p472a.p473d.Sake.Isakeclient
    ...
    public byte[] decrypt(byte[] bArr) {
        ...
        if (z) {
            ...
            z = SakeJNIWrapper.isInsecureAfterReceive(this.nativeSakeClient, nativeSakeSecureMessage, nativeSakeUserMessage);
            if (!z && this.clientStatus == SakeClientStatus.AUTHORIZED) {
                setStatus(SakeClientStatus.LINK_SYNC_LOST);
            }
        } else {
        ...
        if (z) {
            return SakeJNIWrapper.cdataUint(nativeSakeUserMessage.m24077d(), (int) nativeSakeUserMessage.getByteCount());
        }
        return null;
    }
   ...
   // @Override // p123e.p416g.p471f.p472a.p473d.Sake.Isakeclient
    ...
    public byte[] doHandshake(byte[] bArr) {
        ...
        if (bArr != null) {
            ...
            p_uint8_t p_uint8_tVar = new p_uint8_t(bArr.length);
            SakeJNIWrapper.memMove(SakeJNIWrapper.getPointer(p_uint8_tVar.GetAsPointer()), min < bArr.length ? Arrays.copyOf(bArr, min) : bArr);
            nativeSakeSecureMessage = new NativeSakeSecureMessage();
            nativeSakeSecureMessage.setByteCount(bArr.length);
            nativeSakeSecureMessage.setBytes(p_uint8_tVar.GetAsPointer());
        } else {
        ...
        SakeHandshakeStatus m3607b = SakeJNIWrapper.getStatus(this.nativeSakeClient, nativeSakeSecureMessage, nativeSakeSecureMessage2);
        ...
        if (nativeSakeSecureMessage2.getSecureMessageByteCount() > 0) {
            return SakeJNIWrapper.cdata(SakeJNIWrapper.getPointer(nativeSakeSecureMessage2.getBytes()), (int) nativeSakeSecureMessage2.getSecureMessageByteCount());
        }
        return null;
    }
    ...
    public boolean initClient(byte[] keydb) {
        ...
        boolean isDbOpen = SakeJNIWrapper.keyDbOpen(nativeSakeKeyDatabase, this.pKeyDatabaseBytes.GetAsPointer(), keydb.length);
